...
object CctvSafeRoutePlanner {
    fun countRelevantCoordinates(
        ...
        return createCandidateNodes(
            ...
            projectedCctvCoordinates = routeContext.projectedCctvCoordinates
                .filter { projectedCoordinate ->
                    projectedCoordinate.coordinate.isRouteEligibleCandidate()
                },
    ...
    fun plan(
        ...
        val projectedCctvCoordinates = routeContext.projectedCctvCoordinates
            .filter { projectedCoordinate ->
                projectedCoordinate.coordinate.isRouteEligibleCandidate() &&
                    projectedCoordinate.coordinate.address !in excludedAddresses
            }
        ...
        val directMetrics = calculateRouteMetrics(
            routeNodes = listOf(start, destination),
            projectedCctvCoordinates = projectedCctvCoordinates,
            projectedStreetlightCoordinates = projectedStreetlightCoordinates
        )
        if (maxWaypointCount == 0 || cctvNodes.isEmpty()) {
            return directPlan(
        ...
        val routeState = findBestRoute(
            ...
            minWaypointCount = MIN_SAFE_ROUTE_WAYPOINT_COUNT
        ) ?: return directPlan(
            metrics = directMetrics,
            candidateCctvCount = cctvNodes.size
        )
        val routeNodes = restoreRouteNodes(routeState, nodes)
        val routeMetrics = calculateRouteMetrics(
        ...
        return CctvSafeRoutePlan(
            ...
            estimatedDistanceMeters = routeMetrics.distanceMeters.toInt(),
            estimatedCost = routeState.gScore,
            estimatedCoverageRatio = routeMetrics.coverageRatio,
            estimatedStreetlightCoverageRatio = routeMetrics.streetlightCoverageRatio,
            candidateCctvCount = cctvNodes.size
        )
    }
    ...
    private fun directPlan(
        ...
        candidateCctvCount: Int
    ): CctvSafeRoutePlan {
        return CctvSafeRoutePlan(
            waypoints = emptyList(),
            estimatedDistanceMeters = metrics.distanceMeters.toInt(),
            estimatedCost = metrics.cost,
            estimatedCoverageRatio = metrics.coverageRatio,
            estimatedStreetlightCoverageRatio = metrics.streetlightCoverageRatio,
            candidateCctvCount = candidateCctvCount
        )
    }
    ...
    private fun calculateRouteMetrics(
        ...
        val distanceMeters = segmentMetrics.sumOf(SegmentMetrics::distanceMeters)
        ...
        val cost = segmentMetrics.sumOf(SegmentMetrics::cost)
        return RouteMetrics(
            segmentMetrics = segmentMetrics,
            distanceMeters = distanceMeters,
            cost = cost,
            coverageRatio = if (distanceMeters > 0.0) {
                weightedCoverage / distanceMeters
            } else {
                1.0
            },
            streetlightCoverageRatio = if (distanceMeters > 0.0) {
                weightedStreetlightCoverage / distanceMeters
            } else {
                1.0
            }
        )
            ...
            .mapNotNull { projectedCoordinate ->
                ...
                RouteNode.cctv(
                    ...
                    candidateRank = distanceFromDirectPath -
                        ...
                        projectedCoordinate.coordinate.routePriority *
                        SUPPLEMENTAL_CCTV_ROUTE_PENALTY_METERS
                )
            }
    ...
    private fun findBestRoute(
        ...
        while (openSet.isNotEmpty()) {
            ...
            if (
                current.state.nodeIndex == destinationIndex &&
                current.state.waypointCount >= minWaypointCount
            ) {
                return SearchResult(
            ...
            neighborIndexes(
                ...
                val tentativeGScore = current.gScore +
                    segment.cost +
                    if (neighbor.coordinate == null) {
                        0.0
                    } else {
                        WAYPOINT_COST_METERS +
                            neighbor.coordinate.routePriority *
                            SUPPLEMENTAL_CCTV_ROUTE_PENALTY_METERS
                    }
    ...
    private fun neighborIndexes(
        ...
        return buildList {
            if (currentState.waypointCount >= minWaypointCount) {
    ...
    private data class RouteMetrics(
        val segmentMetrics: List<SegmentMetrics>,
    ...
    private const val MIN_SAFE_ROUTE_WAYPOINT_COUNT = 1
    private const val SUPPLEMENTAL_CCTV_ROUTE_PENALTY_METERS = 350.0
